...
Model parameters are serialized using Python's \texttt{pickle} module. The saved state dictionary stores weights and bias values with keys following the format \texttt{layer\_\{i\}\_weights} and \texttt{layer\_\{i\}\_bias}, ensuring compatibility with the reconstructed network architecture.

\textbf{Training Process:}  
Training is performed using manual forward and backward propagation across layers. Gradient descent is applied with a configurable learning rate, and loss values are monitored during training iterations.

\textbf{Network Architecture:}  
The network supports flexible architectures with multiple hidden layers. Layers are composed sequentially using the \texttt{add()} method, allowing different activation functions and layer depths to be evaluated experimentally.
